...
“系统设置 → 系统状态监控”使用一个状态按钮控制：绿色“已开启”、灰色“已关闭”，每点一次切换一次。开启后，KEMI Pads 只在活动副屏创建一个半透明圆角悬浮窗，每 3 秒更新整机 8 核合计 CPU、各核心占用与实时频率、内存和交换分区。悬浮窗可自由拖动，位置会被记住；轻点后分页查看 App、系统服务、原生进程和内核线程，继续轻点翻页并在末页后折叠。
